...
class APNS_DB {
  ...
  static public function get_active_devices() {
    return db::$connection->query("SELECT device_id FROM AppleDevice where active = 1");
  }
  ...
  static public function get_unread_notifications($device_id) {
    $device = self::get_device($device_id);
    $unreads = $device['unread_notifications'];
    return $unreads;
  }

  static public function mark_notifications_as_read($device_id, $tags) {
    $unreads = self::get_unread_notifications($device_id);
    foreach ($tags as $tag) {
      $position = array_search($tag, $unreads);
      if ($position !== false) {
	array_splice($unreads, $position, 1);
      }
    }
    self::save_unread_notifications($device_id, $unreads);
  }
